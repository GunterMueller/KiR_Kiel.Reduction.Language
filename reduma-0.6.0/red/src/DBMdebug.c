...
int DBMncreadyproc(proc, block)
  ...
  if (block) {
    ...
    nread(fdcube, NULL, 0, &lproc, &type);
    dbg_ready[proc] = 1;
    DBUG_RETURN(type == MT_DBG_READY ? 1 : -1);
    }
  else if (ntest(fdcube, &lproc, &type) >= 0) {
...
int DBMgetschedinfo(proc, schedinfo)
  ...
  sz = nread(fdcube, (char *)schedinfo, sizeof(*schedinfo), &lproc, &type);
...
int DBMgetpidinfo(proc, pid, procinfo)
  ...
  sz = nread(fdcube, (char *)procinfo, sizeof(*procinfo), &lproc, &type);
  DBUG_PRINT("DEBUG", ("answer received, size %d", sz));
...
int DBMgetstack(proc, stack, off, size, stackbuf)
  ...
  if (size == 0) {
    ...
    DBUG_PRINT("DEBUG", ("request for stack (from %d size %d) slice sent to %d",
        off, size, proc));
    sz = nread(fdcube, (char *)stackbuf, size * sizeof(STACKELEM), &lproc, &type);
    DBUG_PRINT("DEBUG", ("answer received, size %d", sz));
    }
...
int DBMgetcode(proc, code, size, codebuf)
  ...
  if (size == 0) {
    ...
    DBUG_PRINT("DEBUG", ("request for code (from %x size %d) sent to %d",
        code, size, proc));
    sz = nread(fdcube, (char *)codebuf, size * sizeof(STACKELEM), &lproc, &type);
    DBUG_PRINT("DEBUG", ("answer received, size %d", sz));
    }
...
int DBMgetdesc(proc, desc, descbuf)
  ...
  sz = nread(fdcube, (char *)descbuf, sizeof(T_DESCRIPTOR), &lproc, &type);
  DBUG_PRINT("DEBUG", ("answer received, size %d", sz));
...
int DBMgetheap(proc, heap, size, heapbuf)
  ...
  if (size == 0) {
    ...
    sz = nread(fdcube, (char *)heapbuf, size * sizeof(PTR_HEAPELEM), &lproc, &type);
    DBUG_PRINT("DEBUG", ("answer received, size %d", sz));
    }
...
int DBMncpidterminated(proc, pid)
  ...
  nread(fdcube, (char *)&isterm, sizeof(isterm), &lproc, &type);
...
int DBMncterminated()
  ...
  DBUG_ENTER("DBMncterminated");
  type = MT_POST_MORTEM;
  if (ntest(fdcube, &lproc, &type) >= 0) {
    DBUG_PRINT("DEBUG", ("POST_MORTEM send by %d", lproc));
    DBUG_RETURN(1);
    }
  type = MT_END_RESULT;
  if (ntest(fdcube, &lproc, &type) >= 0) {
    DBUG_PRINT("DEBUG", ("END_RESULT send by %d", lproc));
    DBUG_RETURN(1);
    }
  DBUG_RETURN(0);
  }
