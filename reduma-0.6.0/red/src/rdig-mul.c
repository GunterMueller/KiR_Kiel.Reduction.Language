/* $Log: rdig-mul.c,v $
 * Revision 1.4  1995/05/22  09:07:47  rs
 * changed nCUBE to D_SLAVE
 *
 * Revision 1.3  1993/09/01  12:37:17  base
 * ANSI-version mit mess und verteilungs-Routinen
 *
 * Revision 1.2  1992/12/16  12:49:38  rs
 * ANSI-Version
 *
...
PTR_DESCRIPTOR desc1, desc2;
{
   ...
   int          i/*,j*/, /* RS 30/10/92 */      /*     counter variables      */
...
/*if (debconf) DescDump(stderr,desc2,1);*/
  ...
  if (*op2_start == 1  &&  ndigit_op2 == 1)     /* 2.argument equal to 1 or ~1*/ 
  {
      ...
      if (R_DESC((*desc1),ref_count) == 1  &&  _digit_recycling)
      {
          ...
          INC_REFCNT(desc1);
          DBUG_RETURN(DIGIT_TEST(desc1,"digit_mul",desc1,desc2));
      }
      else
      {
          ...
          for ( i=0; i<ndigit_op1; i++)
              *ptr_res++ = *ptr_op1++;
          REL_HEAP;
          DBUG_RETURN(DIGIT_TEST(res_desc,"digit_mul",desc1,desc2));
      }
  }
