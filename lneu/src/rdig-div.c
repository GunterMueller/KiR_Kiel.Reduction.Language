...
L_eine_ziffer:
...
#if DEBUG
  if (debconf) printf("div: Divisor einstellig. ndigit1:%d, ndigit2:%d, wert:%d\n",ndigit1,ndigit2,wert);
#endif
 ...
 if (debconf) printf("div: maxdigit:%d\n",maxdigit);
      ...
      {                                       /* korrigieren */
         ...
         temp = (temp % wert) * BASE;
#if DEBUG
 if (debconf) printf("div: 1. Ergebnis:%d, Rest:%d\n",data3[cneu],temp);
#endif
         cneu--; calt--;
      }
      else
      {
         ...
         exp--;
#if DEBUG
 if (debconf) printf("div: 2. Ergebnis:0, Rest:%d\n",temp);
#endif
         cneu--; calt--;
      }
      ...
      while (calt >= 0 && cneu >= 0)              /* Noch Ziffern im Dividenden */
      {
         ...
         temp = (temp % wert) * BASE;
#if DEBUG
 if (debconf) printf("div: 3. Ergebnis:%d, Rest:%d\n",data3[cneu],temp);
#endif
         cneu--; calt--;
      }
      ...
      {                                     /* aber Rest vorhanden und nicht */
         ...
         temp = (temp % wert) * BASE;
#if DEBUG
 if (debconf) printf("div: 4. Ergebnis:%d, Rest:%d\n",data3[cneu],temp);
#endif
  ...
  if (debconf) printf("div: Division endlich. cneu:%d, maxdigit:%d\n",cneu,maxdigit);
    ...
    if (debconf) printf("div: cutheap durchgefuehrt\n");
      ...
      return(DIGIT_TEST(res,"digit_div",desc1,desc2));
 ...
 *****************************************************************************/
  ...
  if (debconf) printf("div: Eingabezahlen normiert\n");
   ...
   {                                        /* -> Ergebnis = 0              */
      ...
      DIGNULL;
      INC_REFCNT(_dig0);
      return(_dig0);                        /* anderen Digit-Routine zu ver- */
   }                                        /* hindern.                      */
   ...
   if ((res = newdesc() ) == 0)
   {
      ...
      DEC_REFCNT(zw2);                      /* um das recycling in einer     */
      ABBRUCH;
      return((PTR_DESCRIPTOR)0);            /* anderen Digit-Routine zu ver- */
   }                                        /* hindern.                      */
   DESC_MASK(res,1,C_DIGIT,TY_DIGIT);       /* Deskriptor initialisieren */
   if (newheap( maxdigit, A_DIGIT((*res),ptdv)) == 0)
   {
 ...
 if (debconf) printf ("div: _dig0:%x, zw1:%x, zw2:%x, ndigit1:%d, ndigit2:%d\n",_dig0,zw1,zw2,ndigit1,ndigit2);
  ...
  if (debconf) printf("div: v1:%d, v2:%d\n",v1,v2);
#endif
   ...
   {                                        /* handen u. Rest != null       */
      ...
      {                                              /* Ziffer korrigieren */
         q--;
#if DEBUG
 if (debconf) printf("div: q wird minimiert: q:%d\n",q);
#endif
      }

#if DEBUG
  if (debconf) printf("div: uj:%d, uj1:%d, uj2:%d, q:%d, ndigit:%d\n",uj,uj1,uj2,q,ndigit);
#endif
...
#if DEBUG
  if (debconf) printf("**** div: Eingetragen q:%d, cneu:%d, calt:%d\n",q,cneu,calt);
#endif
      cneu--;
      calt--;
      exp_lauf--;
