...
{
    int     i,arity;
    STACKELEM var;                 /* fuer das POP_I_BLOCK Makro */
    int     pm_arity;
...
/*******************************************************************/
        ...
        /*--------------------------------------------*/
            ...
            /*----             kann der Editor nicht  !!!                ----*/
                   ...
                   {
                     PUSHSTACK (S_i,arg);
                     if (T_POINTER (READSTACK (S_i)))
                         INC_REFCNT ((PTR_DESCRIPTOR) READSTACK (S_i));
                   }
...
/*        for ( ; arity > 0; --arity)
                                      ...
                                      Listendeskriptoren */
        {
            ...
            {                     /* leere Liste (oder String) zurueckgeben */
                ...
                {
                    PUSHSTACK(S_i, (STACKELEM) _nilstring);
                    INC_REFCNT(_nilstring);
                }
                else
                {
                    PUSHSTACK(S_i, (STACKELEM) _nil);
                    INC_REFCNT(_nil);
                }
            }
            i += 3;
    ...
    {                                                     
        ...
        POP_I_BLOCK(var);              /* angelegten Block wieder beseitigen */
        END_MODUL ("pm_am");
        REL_HEAP;
