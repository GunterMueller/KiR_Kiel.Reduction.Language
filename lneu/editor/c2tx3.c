...
#define APARMS	348	/* size of ascii representation of parms	*/
...
PTR_UEBERGABE  p_parms;
{
  ...
  switch (p_parms->redcmd) {
    ...
    case RED_NP:  /* get next page	*/
    case RED_RD:  /* get last page, call reduct	*/
    {
      register int i;
      ...
      for ( ; i > 0; i--)
      {
        PPUSHSTACK(S_a,(*lauf++)); /* push stack elements onto A	*/
      }

      if (p_parms->redcmd == RED_NP)
      {
        restore_state(old_reduma_state);
        return;	/* there is still something left	*/
      }
      /* now reduce	*/
      ...
      _base     = p_parms->base;
      _maxnum   = _base - 1;
      /* convert external precision into internal	*/
    ...
    case RED_NRP:   /* output next page of reduced expression */
    {
       register int           i  = 0;
       register int         size = SIZEOFSTACK(S_e);
       register STACKELEM  *lauf = stack;

       if (size == 0)   /* error which should not happen */
       {
