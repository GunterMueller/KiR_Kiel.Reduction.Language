...
extern int EXT_INPUT_MODE; /* UDI : defined in cinput.c */
...
pickexpr(st)
 STACK *st;
 {
  ...
  res = pick(st);
  if (res == RDOK) {
    if (!EXT_INPUT_MODE) message(MROW,MCOL,"Picked up CE");
   }                                        else 
  if (res == EXNPAR) {
    if (!EXT_INPUT_MODE) message(MROW,MCOL,"Can't pick NOPAR");
    result=0;
   }                                        else 
   {
    if (!EXT_INPUT_MODE) message(MROW,MCOL,"pick: Unknown error");
    result=0;
   }    
  if (!EXT_INPUT_MODE) getunget();  
  if (!EXT_INPUT_MODE) clrmsg(MROW,MCOL);
  return (result);
 }
/* ------------- pick -------------------------------- */
/* kopieren der CE auf stack st                        */
