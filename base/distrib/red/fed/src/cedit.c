...
#define ZIEMLICH_VIEL	1000000000                   /* stt 15.02.96 */
...
static void usage()
 {
  ...
  fputs(" -P: print all files in paperprint format\n",stdout);
  ...
  fprintf(stdout," -i <file>: use <file> for initialisation [default %s]\n",
          initfile);
  fputs(" -x <source>: read interactions std.input from <source>\n",stdout);
  fputs(" -y <target>: write interactions std.output to <target>\n",stdout);
 }
/* -------- init_back ------------------------------ */
...
editor(state)
 P_EditState state;
 {
  ...
  extern char *ia_instream, *ia_outstream;
  long int initedit(),initparms(),newdate;
  int compare,time,preld,v_opt,erg,print,Print,out;
  register int c,ok;
  register char *s;
  int argc,lastc;
...
*/
    ...
    Print     = FALSE;	/* ausgabe von dateien in paper-Format    */
    msg[0]    = '\0';
     ...
     for (s = argv[0]+1; *s != '\0'; s++) {
      switch (*s) {
        ...
        case 'x':
          ...
          --argc;
          if (argc > 0  &&  *s) {
            if ((ia_instream=(char *)malloc(strlen(s)+1)) != NULL)
              strcpy(ia_instream,s);
            else {
              fprintf(stdout,"%s: no memory for ia_instream\n",exednm);
              return(FAILURE);
            }
          }
          else {
            fprintf(stdout,"%s: missing stdin-source\n",exednm);
            return(FAILURE);
          }
          *s-- = '\0';
          break;
        case 'y':
          ...
          --argc;
          if (argc > 0  &&  *s) {
            if ((ia_outstream=(char *)malloc(strlen(s)+1)) != NULL)
              strcpy(ia_outstream,s);
            else {
              fprintf(stdout,"%s: no memory for ia_outstream\n",exednm);
              return(FAILURE);
            }
          }
          else {
            fprintf(stdout,"%s: missing stdout-target\n",exednm);
            return(FAILURE);
          }
          *s-- = '\0';
          break;
    ...
    if (print || Print) {
      ...
      art = Print ? PPREPLACE | PPPAPER : PPREPLACE;
      ...
      while (argc > 0) {
        ...
        c = rdfile(fn1);
        if (!RD_OK(c)) return(FAILURE);
        if (prettyprint(fn1,"w",width,art,FN_PP) != SUCCESS) {
