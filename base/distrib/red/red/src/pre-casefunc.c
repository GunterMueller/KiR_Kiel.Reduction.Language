 ...
 * Revision 1.22  2001/07/02 15:32:29  base
 * compiler warning eliminated
 *
 * Revision 1.21  2001/03/13 16:06:17  base
 * RCS header modified
 *
 * Revision 1.20  2001/03/13 16:05:27  base
 * signaure of READ_M, READ_E, READ_V modified
 *
 * Revision 1.19  1997/02/17 13:18:12  rs
 * some additions supporting speculative evaluation
 *
 * Revision 1.18  1996/06/14  14:04:50  rs
 * the (famous) F1-problem with some programs should be fixed
 * (there was a problem with unary letp's)
 *
 * Revision 1.17  1996/04/04  16:10:20  rs
 * more DBUG output
 *
 * Revision 1.16  1996/03/07  15:38:22  rs
 * another bugfix concerning the SUB-pattern-descriptor...
 *
 * Revision 1.15  1996/03/06  17:24:17  rs
 * err SUB converted to tilde-SUB !
 *
 * Revision 1.14  1995/08/21  12:29:15  rs
 * re-arranged pm-variables in old switch-descriptor
 * (sorry for the typo in the pmsupport.c log ;-)
 *
...
/* US */
    ...
    if (T_SPEC_CASE(y)) {
      DBUG_PRINT("casefunc", ("Es ist ein spec. Case !!!"));
      this_is_spec = 1; }
...
main_e:
  ...
  if ( T_PM_WHEN(x))
     ...
     /* Pattern fertig bearbeitet => WHEN auf M-Stack dekrementieren:*/
       ...
       /* "toller Programmierstil..." , Anmerkung Jahre danach ... */

       START_MODUL("UH_PM_CONVERSION");
       MKDESC(clausepx,1,C_MATCHING,TY_CLAUSE);
       GET_HEAP(5,A_CLAUSE(*clausepx,sons));  /* Platz fuer das Pattern,Guard,Body Triple + ggf. OLD MATCH DESCRIPTOR + flag for spec and other stuff */
       L_CLAUSE(*clausepx,sons)[3] = NULL;
       L_CLAUSE(*clausepx,sons)[4] = this_is_spec;
        ...
        { 
         if (!when)
            ...
            /*      PUSHSTACK(S_pm,pmpx);         DBUG ONLY :-) */
                 ...
                 DBUG_PRINT("PRE-CASEFUNC", ("j = anz_when - 1 = %d", anz_when - 1));
                 ...
                 for (j = anz_when - 1; j > 0 ; j--)
                   ...
                   {   
                      ...
                      if (j == (anz_when - 1)) {
                        DBUG_PRINT("PRE-CASEFUNC", ("last clausepx: %x", clausepx));
                        L_CLAUSE(*clausepx,next) = NULL;
                        clausepxbak = clausepx;
                        }
                      else {
                        DBUG_PRINT("PRE-CASEFUNC", ("act. clausepx %x, next is %x", clausepx, clausepxbak));
...
/* US */
              ...
              L_CLAUSE(*(PTR_DESCRIPTOR)R_SELECTION(*pmpx,clauses),next) = NULL;
              }
#endif /* UH_ZF_PM */
