...
#define MPRINT_ASCII_PROC_CREATE(F, TIME, PROC, PID, PC_PID, PC_TYPE) \
	fprintf(F, "%.lf %d %d PROC_CREATE %d %d\n", TIME, PROC, PID, PC_PID, PC_TYPE)
#define MPRINT_BINARY_PROC_CREATE(F, TIME, PROC, PID, PC_PID, PC_TYPE) \
	M_TIMER_TYPE_BUF = TIME, fwrite(&M_TIMER_TYPE_BUF, sizeof(M_TIMER_TYPE), 1, F), \
	M_BINARY_COUNT = sizeof(PROCESS_NR_TYPE) + sizeof(PROCESS_PID_TYPE) + sizeof(M_CHAR_BUF) + sizeof(M_INT_BUF) + sizeof(M_INT_BUF), \
	fwrite(&M_BINARY_COUNT, sizeof(M_BINARY_COUNT_TYPE), 1, F), \
	PROCESS_NR_BUF = PROC, fwrite(&PROCESS_NR_BUF, sizeof(PROCESS_NR_TYPE), 1, F), \
	...
	M_INT_BUF = M_SWAP(PC_TYPE), fwrite(&M_INT_BUF, sizeof(M_INT_BUF), 1, F), \
	1
#ifdef M_BINARY
#define MPRINT_PROC_CREATE(F, TIME, PROC, PID, PC_PID, PC_TYPE) \
	MPRINT_BINARY_PROC_CREATE(F, TIME, PROC, PID, PC_PID, PC_TYPE)
#else
#define MPRINT_PROC_CREATE(F, TIME, PROC, PID, PC_PID, PC_TYPE) \
	MPRINT_ASCII_PROC_CREATE(F, TIME, PROC, PID, PC_PID, PC_TYPE)
#endif
