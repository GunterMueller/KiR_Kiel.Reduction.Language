/*
 * $Log: rlstruct.c,v $
 * Revision 1.8  2001/07/02 14:53:33  base
 * compiler warnings eliminated
 *
 * Revision 1.7  1998/09/24 13:12:10  rs
 * bugfixed version for SPEC_TICKETS + some more features
 *
 * Revision 1.6  1997/06/06 09:13:11  rs
...
extern int rout_specmap1, rout_specmap2;
extern int max_spec_sons;
...
register STACKELEM arg1,arg2;
{
      ...
      (T_POINTER(arg2) && (R_DESC(DESC(arg2),class) == C_LIST)))
  {
    ...
    DBUG_PRINT("red_lunite", ("dim: %d, dim2: %d", dim, dim2));
...
/* another function for speculative evalutaion */

int red_specmap(arg1,arg2)
register STACKELEM arg1,arg2;
{
  int i, nfv;
  int help;
  ...
  DBUG_ENTER ("red_specmap");
  START_MODUL("red_specmap");

#if (D_SPEC && D_SLAVE)

  DBUG_PRINT("specmap", ("T_POINTER(arg1): %d, (R_DESC(DESC(arg1),class) == C_FUNC): %d", T_POINTER(arg1), (R_DESC(DESC(arg1),class) == C_FUNC)));
  DBUG_PRINT("specmap", ("T_POINTER(arg2): %d, (R_DESC(DESC(arg2),class) == C_LIST): %d", T_POINTER(arg2), (R_DESC(DESC(arg2),class) == C_LIST)));

  if (T_POINTER(arg1) && 
      (R_DESC(DESC(arg1),class) == C_FUNC) &&
      T_POINTER(arg2) && (R_DESC(DESC(arg2),class) == C_LIST)) {

    DBUG_PRINT("specmap", ("list-length of arg2 is: %d", R_LIST(DESC(arg2),dim)));
    ...
    if (curr_pid->son_results == NULL) {
      ...
      DBUG_PRINT("specmap", ("initialize ->son_* pointers !"));
    ...
    /* now start len(arg2) speculative processes... */

    help = R_LIST(DESC(arg2),ptdv);

    for (i=0; i<R_LIST(DESC(arg2),dim); i++) {

      DBUG_PRINT("specmap", ("preparing a speculative thread... list entry %d", i));

      PUSH_A(*((int *)help));
      if (T_POINTER((T_PTD)(*((int *)help))))
        INC_REFCNT((T_PTD)(*((int *)help)));
      desc = TOP_W();
      PUSH_A(desc);
      INC_REFCNT(desc);
      /* no everything is prepared for a speculative call I think ... */

#if D_SPEC_TICKETS

    ptdesc = create_spec_frame(0, 0, 2);
 
/* fprintf(debugger, "0x%x: LISTDEC allocates 0x%x as descriptor !\n", curr_pid, ptdesc); fflush(debugger);  */
 
    DBUG_PRINT("specmap", ("specmask1 is 0x%x", curr_pid->specmask1));
 
    SPEC_SF_GUARD(*ptdesc) = curr_pid->son_status[0];
 
    DBUG_PRINT("specmap", ("GUARD grabbed, it is 0x%x ! MUST BE NULL !", SPEC_SF_GUARD(*ptdesc)));
 
    curr_pid->son_results[PC_GET_SPEC_SPAWNED(curr_pid)] = ptdesc;
    PC_SET_SPEC_SPAWNED(curr_pid, PC_GET_SPEC_SPAWNED(curr_pid)+1);
 
    DBUG_PRINT("specmap", ("new spec-frame %x generated, %d frames stored altogether", ptdesc, PC_GET_SPEC_SPAWNED(curr_pid)));
 
    curr_pid->son_status[0] = NULL; /* push the flag for TGUARD */

#else

      spec_distribute (0,2,0);
    ...
    DBUG_PRINT("specmap", ("preparing for microcode..."));
    PUSH_R(code);

    DBUG_PRINT("specmap", ("old code: %x", code));

    code = &rout_specmap1;
 
    DBUG_PRINT("specmap", ("new code: %x", code));
