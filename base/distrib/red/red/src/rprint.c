/*
 * $Log: rprint.c,v $
 * Revision 1.19  2001/07/02 14:53:33  base
 * compiler warnings eliminated
 *
 * Revision 1.18  1998/02/18 13:34:36  rs
 * modifications for speculative evaluations regarding
 * meaning preserving transformations
 *
 * Revision 1.17  1996/02/28  11:02:30  stt
 * output of TO_CHAR, TO_ORD, SPRINTF and SSCANF added.
 *
 * Revision 1.16  1996/02/16  17:59:31  stt
 * In prnsingleatom() use intact_tab[] now.
 *
 * Revision 1.15  1996/01/08  15:36:59  stt
 * new interactions bind and unit inserted
 * fstatus renamed to finfo
 *
 * Revision 1.14  1995/12/11  18:25:10  cr
 * support for empty frames: NILSTRUCT
 *
 * Revision 1.13  1995/12/11  13:13:46  cr
 * two more operations on frames: FR_SLOTS,FR_DELETE
 *
 * Revision 1.12  1995/11/15  15:36:27  cr
 * introduced _STRUCT,_ENTRY,FR_TEST,FR_SELECT,FR_UPDATE
 *
...
int   flag;
{
  ...
  if ((temp.element & P_VAR_CONSTR) == VAR_CONSTR)
    ...
    switch ((temp.element & F_CONSTR_NAME) >> O_CONSTR_NAME)
    {
      ...
      case _LIST:
#if D_SPEC
        if (T_SPECLIST1(temp.element))
          sprintf (hilf, CTRLSTR2, "Specl1 ", ARITY (temp.element));
        else
#endif
...
/* H */
      case _PM_SWITCH:                                    /* pattern - match */
        ...
        strcat (line, hilf);
        return;
/*    case _STRUCT:        Frame, cr 15.11.95 */
      case _PM_AS:                                        /* pattern - match */
        if (T_CHAINED_CONSTR(temp.element))
          sprintf (hilf, CTRLSTR3, "Frame  ", ARITY (temp.element), 2);
        else
   ...
   /* case _PM_NCASE :
        ...
        strcat (line, hilf);
        return; */
/*    case _ENTRY:        Slot in Frame, cr 15.11.95 */
      case _PM_LIST:   /* pattern - match */
        if (T_CHAINED_CONSTR(temp.element))
          sprintf (hilf, CTRLSTR3, "Slot   ", ARITY (temp.element), 2);
        else
...
/* ak *//* UNPROTECT macht genau ein PROTECT ungueltig */
      case _UNPROTECT :                       
        ...
        return;
      case _SPECLIST2:
        sprintf (hilf, CTRLSTR2, "Sp.2   ", ARITY (temp.element));
        strcat (line, hilf);
        return;
...
int   flag;
{
  ...
  switch ((P_FUNC & Top.element))
  {
    case FUNC:                                             /* eine  Funktion */
    {
      ...
      if (T_IAFUNC(Top.element))
      {
        sprintf(hilf, CTRLSTR1, intact_tab[FUNC_INDEX(Top.element)].name);
        strcat (line, hilf);
        return;
      }
      else switch (Top.element)
      {
        case M_NOT:
        ...
        /* frame operations FR_..., cr  15.11.95 */

        case FR_TEST:                               /*  frame operations    */ 
             sprintf (hilf, CTRLSTR1, "ftest     ");/*                      */
             strcat (line, hilf);
             return;    
        case FR_SELECT:                             /*  frame operations    */ 
             sprintf (hilf, CTRLSTR1, "fselect   ");/*                      */
             strcat (line, hilf);
             return;    
        case FR_UPDATE:                             /*  frame operations    */ 
             sprintf (hilf, CTRLSTR1, "fupdate   ");/*                      */
             strcat (line, hilf);
             return;    
        case FR_SLOTS :                             /*  frame operations    */ 
             sprintf (hilf, CTRLSTR1, "fslots    ");/*  04.12.95            */
             strcat (line, hilf);
             return;    
        case FR_DELETE:                             /*  frame operations    */ 
             sprintf (hilf, CTRLSTR1, "fdelete   ");/*  04.12.95            */
             strcat (line, hilf);
             return;    

        /* stt 28.02.96 */
        case TO_CHAR:
             sprintf (hilf, CTRLSTR1, "char      ");
             strcat (line, hilf);
             return;
        case TO_ORD:
             sprintf (hilf, CTRLSTR1, "ord       ");
             strcat (line, hilf);
             return;
        case SPRINTF:
             sprintf (hilf, CTRLSTR1, "sprintf   ");
             strcat (line, hilf);
             return;
        case SSCANF:
             sprintf (hilf, CTRLSTR1, "sscanf    ");
             strcat (line, hilf);
             return;
...
/* mah 011193 */
      ...
      switch (Top.element)
      {
          ...
          case NILSTRING:                            /*  empty string       */
               ...
               strcat (line, hilf);
               return;
          case NILSTRUCT:                            /*  empty frame, cr 11.12.95 */
               sprintf (hilf, CTRLSTR1, "NILSTRUCT ");
               strcat (line, hilf);
               return;
          case CL_FUNCTION:
