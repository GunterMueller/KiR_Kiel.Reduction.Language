/*
 * $Log: rheap.c,v $
 * Revision 1.27  2001/07/02 14:53:11  base
 * compiler warnings eliminated
 *
 * Revision 1.26  1997/06/06 09:13:11  rs
...
#define FREEDESC(desc)        { if (desc != NULL)  {       \
                                HEAP_CHAIN(desc) = deslist; \
    DBUG_PRINT("FREEDESC", ("freeing %x !", desc)); \
                                deslist = desc; descnt--; }}
#endif
...
/*        ASSERT(StHeapD<=p && p<=EnHeapD,"address not desc area"); */
	...
	if (StHeapD>p || p>EnHeapD)
          post_mortem("address not desc area");
/*	  printf("AU SCHEISSE"); */
...
DBUG_ENTER ("freedesc");
        ...
        DBUG_PRINT("freedesc", ("freeing %x !", p));
