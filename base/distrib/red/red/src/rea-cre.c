...
extern int mvheap();                        /* rhinout.c mah 100893 */
...
#ifdef AUTODIST
  if (d_autodist_on)
    d_autodist();     /* syntactic changes for automatic distributing expressions */
#endif 
...
main_m:
  ...
  {                                                  /* auslagern            */
     ...
     _bindings -= ARITY(x);                       /* Anzahl offener Bindungen */
                                            /* Steht im PRELIST */
     if (mvheap(A_FUNC((*((PTR_DESCRIPTOR)READSTACK(S_a))),pte)) == 0)
       post_mortem ("ea_create: heap out of space"); /* Auslagern */
     /* important change (first change) from mah  10.8.1993 */ 
