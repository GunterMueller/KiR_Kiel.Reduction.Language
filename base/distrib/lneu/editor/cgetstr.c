...
getletstr(eno,pts,firstmark)
 ...
 {
  STACKELEM callkon();
  register int c,ispredef,index,nokwrd,expected,isinteraction,index2;
  ...
  *pts     = skipblanks(*pts);
  ...
  isinteraction = isalpha(c) &&
                  ((index2 = ispredfct(*pts,interactions,nintact)) != FAILURE);
  ...
  if ((isalpha(c) || c == EXTRENN) &&  (expected = isvar(etptb[eno].exptp))  &&
      ...
      ) && (ispredef || isinteraction))) {
    if (ispredef)
      push(BST,functions[index].predelem | toedit6(eno));
    else
    if (isinteraction)
      push(BST,interactions[index2].predelem | toedit6(eno));
    else
    ...
    if (isinteraction) return(fehl(FPRED,eno));  else
