...

{
  ...
  STACKELEM   case_name, *name,*bv_list;
  PTR_HEAPELEM              offset,hilf;
  PTR_DESCRIPTOR                desc,pf;
  PTR_DESCRIPTOR               sub_desc;                     
  int         anzahl,arity,i,j,anz_when;
  int                            anz_bv;  
  int                         test = 0 ;
  free_var_list                  *hilf1;
  int                         otherwise;
    ...
    /* also ein case-Ausdruck */
     ...
     if ((arity == 1) || when || end )  /* d.h.,SWITCH(1) auf dem M_Stack ,oder */
        ...
        { 
         if (!when)
            {
             POPSTACK(S_m1);  /* SWITCH(n) */
             POPSTACK(S_m);   /* SWITCH(1) */
     ...
     if ( ( i=test_cond() ) == 0 )
      {
        ...
        /* wird mittels condfunc() geschlossen               */
           ...
           /* ein Conditional ,welches geschlossen wird , enthaelt selbst keine */
           /* gebundenen Variablen => auftretende Var. sind rel. frei oder frei */
              fvlist_end();
              condfunc();
              goto main_e;
      }
  ...
  if (T_PM_WHEN(x) || T_PM_OTHERWISE(x))
     {
      if (arity > 1)
            ...
            /* fertig bearbeitet                                    */
        {
         ...
         POPSTACK(S_m);    /* WHEN(1) bzw OTHERWISE(1) */
         POPSTACK(S_m1);   /* WHEN(3) bzw OTHERWISE(1) */
