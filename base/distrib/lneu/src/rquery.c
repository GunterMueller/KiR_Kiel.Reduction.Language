...
/*  if (stop_on_error) 080993 mah: auskommentiert
                              mah: da die aktuelle Editorversion
                              mah: stop_on_error und stop_on_stop nicht
                              mah: mehr unterstuetzt d.h sie sind immer gesetzt
                              mah: und damit zum Beispiel
                              mah: (1+(2+x)) zu (2+x) reduziert wird.
      do_fail(where_to_put);  mah*//* leider wegen dirty trick TOS+1 */ 
