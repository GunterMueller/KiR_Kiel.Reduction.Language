...
static short father[MAX_PROCESSORS][MAX_PROCESSES][STACK_SIZE],
             ...
             newNode[MAX_PROCESSORS][MAX_PROCESSES][MAX_PROCESSORS][MAX_TICKETS],
             topOfNewNodeQueue[MAX_PROCESSORS][MAX_PROCESSES][MAX_PROCESSORS];
...
if(!strcmp(R_TYPE, "MT_NEW_PROCESS")) {
	...
	if(k) {
		/* more than 1 element in the queue */
		actualNode[PROCESSOR][tempProcess]=
          		newNode[R_FROM_PROC][tempProcess2][PROCESSOR][0];
		for(i=0; i<k; i++){
			newNode[R_FROM_PROC][tempProcess2][PROCESSOR][i]=
			newNode[R_FROM_PROC][tempProcess2][PROCESSOR][i+1];
		}
	}
	else {
		actualNode[PROCESSOR][tempProcess]=
