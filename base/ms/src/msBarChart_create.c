...
String *app_defaults;
{
	...
	for (i = 0; i < XtNumber(WI_ms_bar_chart_classes); i++)
		WcRegisterClassPtr(new_context,
			WI_ms_bar_chart_classes[i].class_name,
			*(WI_ms_bar_chart_classes[i].class));

	WcAllowDuplicateConstructorReg(TRUE);
	for (i = 0; i < XtNumber(WI_ms_bar_chart_constructors); i++)
		WcRegisterConstructor(new_context,
			WI_ms_bar_chart_constructors[i].constructor_name,
			WI_ms_bar_chart_constructors[i].constructor);

	WcAllowDuplicateCallbackReg(TRUE);
	register_names(new_context, WI_ms_bar_chart_callback_names,
			XtNumber(WI_ms_bar_chart_callback_names), FALSE);
	register_names(new_context, WI_ms_bar_chart_action_names,
			XtNumber(WI_ms_bar_chart_action_names), TRUE);
	return (dpy);
}
...
String app_defaults[];
{
	...
	dpy = XukcToolkitInitialize(dpy_str, "msBarChart", "MSBarChart",
					argc, argv, options, num_options,
					app_defaults);
	new_shell = XtAppCreateShell("msBarChart", "MSBarChart",
...
{
	create_connections(WI_ms_bar_chart_widget_connections,
			XtNumber(WI_ms_bar_chart_widget_connections));
}
...
Widget toplevel_shell;
{
	...
	WI_ms_bar_chart = toplevel_shell;

	/* create all the widgets in the user interface */
	WcWidgetCreation(toplevel_shell);

	/* bind the widget ids. to variables */
	for (i = 0; i < XtNumber(WI_ms_bar_chart_bind_list); i++) {
		*(WI_ms_bar_chart_bind_list[i].w) = WcFullNameToWidget(toplevel_shell,
			WI_ms_bar_chart_bind_list[i].str);
		if (*(WI_ms_bar_chart_bind_list[i].w) == NULL) {
			printf("WI_ms_bar_chart: failed to bind user interface.  Missing defaults file?\n");
			exit(1);
		}
	}
