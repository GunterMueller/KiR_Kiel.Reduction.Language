...
#if D_SPEC
extern int ack_red_count, max_spec_sons, max_spec_procs, spec_failcase, spec_multi_results, spec_mspecial, spec_kill_no_steps, spec_nondet, nospecnr, spec_retrans;
#endif /* D_SPEC */
...
#if D_SPEC
 DO_SEND(ack_red_count);
 DO_SEND(max_spec_sons);
 DO_SEND(max_spec_procs);
 DO_SEND(spec_failcase);
 DO_SEND(spec_multi_results);
 DO_SEND(spec_mspecial);
 DO_SEND(spec_kill_no_steps);
 DO_SEND(spec_nondet);
 DO_SEND(nospecnr);
 DO_SEND(spec_retrans);
#endif /* D_SPEC */
...
#if D_SPEC
                                 if (send_instr.instruction == I_SPECDIST)
                                    {DO_SEND(((PTR_HEAPELEM)(*heap_ptr++)) - code_address);
                                     DO_SEND(((PTR_HEAPELEM)(*heap_ptr++)) - code_address);
                                     DO_SEND(*heap_ptr++);
                                     DO_SEND(*heap_ptr++);
                                  ...
                                  if ((send_instr.instruction >= I_APPEND) && (send_instr.instruction <= I_MATCHDIGIT)) {
                                    switch(send_instr.instruction) {
                                      ...
                                      case I_BINDSUBL:
                                        ...
                                        break;
                                      default:
                                        DBUG_PRINT("MSG", ("sending UNKNOWN PM-ASM-command: %d", send_instr.instruction));
                                        post_mortem("sending UNKNOWN PM-ASM-code !");
                                        break;
                                      }
                                    break;
                                    }
...
#if D_SPEC_TICKETS
 DBUG_PRINT ("wait_for_ncube", ("Sending spec_ticket_file_name !"));
 dest = 0xffff;
 type = 1;
 nwrite (cube,spec_ticket_file,strlen(spec_ticket_file)+1,dest,type);
 DBUG_PRINT ("wait_for_ncube", ("It's away !"));
