/* $Log: rdig-add.c,v $
 * Revision 1.4  1995/05/22 09:04:46  rs
 * changed nCUBE to D_SLAVE
 *
 * Revision 1.3  1993/09/01  12:37:17  base
 * ANSI-version mit mess und verteilungs-Routinen
 *
 * Revision 1.2  1992/12/16  12:49:34  rs
 * ANSI-Version
 *
    ...
    if (R_DIGIT((*desc1),ptdv)[0] == 0) {
      ...
      if (R_DIGIT((*desc2),sign) == PLUS_SIGN) {
...
#endif
        DBUG_RETURN(DIGIT_TEST(desc2,"digit_add",desc1,desc2));
       }
        ...
        if (R_DESC((*desc2),ref_count) == 1  && _digit_recycling) {
          ...
          REL_HEAP;
          DBUG_RETURN(DIGIT_TEST(res,"digit_add",desc1,desc2));
        }
      }
    }   /* end if 1. arg == 0      */
    /* ------------------------------------------------------------- */
...
#endif
       DBUG_RETURN(DIGIT_TEST(desc1,"digit_add",desc1,desc2));
    }
...
/*   :2:  ----                                                            */
  ...
  if (debconf)  printf("\n\n2. Fall\n");
             ...
             if (data3a[ndigit1]) {          /* ganz vorn Uebertrag ? */
               /* "++" --> "+= 1", TB, 4.11.1992 */
               /* "++" funktioniert hier nicht (Komma-Operator in L_DIGIT) */
               L_DIGIT((*res),ndigit) += 1;  /* eine Ziffer mehr */
...
if (debconf)   printf("\n\n7. Fall\n");
          ...
          if (*(--data3) == 0) {        /* erste Zahl war eine Null, und */
             /* "--" --> "-= 1", TB, 4.11.1992 */
             /* "--" funktioniert hier nicht (Komma-Operator in L_DIGIT) */
             L_DIGIT((*res),ndigit) -= 1;  /* die zweite negativ und ihr    */
