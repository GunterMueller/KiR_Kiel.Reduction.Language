...
/* #define M_PRELIMINARY_DEBUG    PRELIMINARY DEBUGGING */
#undef M_PRELIMINARY_DEBUG
...
#endif
    m_input_array[big_counter].m_entry1 = (void *) i;  /* buffer and counter dereference */
...
/*       strncpy(m_line[m_take_me], m_line[m_max_inputs-1], (int) m_line[m_max_inputs-1][0]+1);  */
       for (k = 0; k<=m_line[m_max_inputs-1][0]; k++)
         m_line[m_take_me][k] = m_line[m_max_inputs-1][k];
...
int m_nr;
{
  ...
  sprintf (m_name, "%s%s_uid%ld_%i", m_mesfilepath, m_mesfilehea, getuid(), m_nr);
...
#ifdef M_PRELIMINARY_DEBUG
  if ((m_binary_counter < 1) || (m_binary_counter > 15))
    fprintf(stderr, "%d announces: length %d\n", proc_id, m_binary_counter);
#endif /* M_PRELIMINARY_DEBUG */
...
if ((m_binary_counter == 8) && (m_line[nr][8] != '\0')) 
  fprintf (stderr, "%d GOTCHA !!\n", proc_id);

if ((m_line[nr][6] != '\0') && (m_line[nr][m_binary_counter] != '\0')) {
  fprintf (stderr, "%d meldet: PRIMA schon beim Einlesen !\n", proc_id);
  }
...
#ifdef M_PRELIMINARY_DEBUG /* PRELIMINARY DEBUGGING */

if ((m_line[number][6] != '\0') && (m_line[number][(int) m_line[number][0]] != '\0')) {
  fprintf (stderr, "%d meldet: PRIMA !\n", proc_id);
  }

#endif /* PRELIMINARY DEBUGGING */
...
#ifdef M_PRELIMINARY_DEBUG /* PRELIMINARY DEBUGGING */
/*   if (m_line[nr][7] == 'h')
    fprintf(stderr, "gotcha: %d chars, |%s|\n", strlen((char *)&m_line[nr][7]), (char *)&m_line[nr][7]); */
     if ((m_line[nr][7] == 'h') && ((int) strlen((char *)&m_line[nr][7]) != 4)) {
       fprintf (stderr, "Es is Laenge %d und Inhalt:%s\n", (int) strlen((char *)&m_line[nr][7]), &m_line[nr][7]);
       fprintf (stderr, "ooops\n"); 
       /* post_mortem ("distributed merging giving up...");*/ }
...
fwrite (&m_line[nr][1], 1, (int) m_line[nr][0], mess_host);
m_line[nr][0] = (char) (m_line[nr][0] + (char) 8);
fwrite (&m_line[nr][0], 1, 1, mess_host);

#ifdef M_PRELIMINARY_DEBUG /* PRELIMINARY DEBUGGING */
 if ((m_line[nr][6] != '\0') && (m_line[nr][0] > 19))
   fprintf (stderr, "backwriting: %d announces: ...pretty much !\n***%s***\n", proc_id, &m_line[nr][7]);
   
 if ((m_line[nr][6] != '\0') && (m_line[nr][(int) m_line[nr][0]-8] != '\0'))
   fprintf (stderr, "backwriting: %d announces: ...unterminated !!!\n***%s***\n",proc_id, &m_line[nr][7]);
...
/*** DBUG DBUG DBUG ***/
    ...
    m_result_buffer[m_result_counter] = '\0';   /* hmmm, is this useful ?! */
    nwrite (m_result_buffer, m_result_counter, m_sendto, type, flags);
...
#ifdef M_PRELIMINARY_DEBUG /* PRELIMINARY DEBUGGING */
 if ((m_result_buffer[m_result_counter+6] != 0) && (m_result_buffer[m_result_counter] > 11))
   fprintf (stderr, "%d announces: ...pretty much !\n***%s***\n", proc_id, &m_result_buffer[m_result_counter+7]);

 if ((m_result_buffer[m_result_counter+6] != 0) && (m_result_buffer[m_result_counter + m_result_buffer[m_result_counter]] != '\0'))
   fprintf (stderr, "%d announces: ...unterminated, length %d, check %d!!!\n***|%s|***\n",proc_id,m_result_buffer[m_result_counter], check_send, &m_result_buffer[m_result_counter+7]);
