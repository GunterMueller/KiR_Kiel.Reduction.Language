 ...
 *
 * Revision 1.7  1996/01/12 13:12:28  rs
 * minor bug fix (red_geq)
 *
 * Revision 1.6  1995/05/22  09:48:03  rs
 * changed nCUBE to D_SLAVE
 *
 * Revision 1.5  1995/04/04  09:37:24  car
 * red_max bug fix
 *
           ...
           switch(R_DESC(DESC(arg2),class)) {
             case C_LIST  :
                  if (R_DESC(DESC(arg2),type) != TY_STRING)
                    goto fail;
                  RES_HEAP;
                  result = (str_lt((PTR_DESCRIPTOR)arg1,(PTR_DESCRIPTOR)arg2) ? SA_FALSE : SA_TRUE);  /* (PTR_DESCRIPTOR) eingefuegt RS 30.11.1992 */     
                  REL_HEAP;
                  goto decrement;
             case C_VECTOR :
             case C_TVECTOR:
