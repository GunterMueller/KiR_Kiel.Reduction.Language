...
int op;         /* eine compare Operation   */
{
  ...
  if T_SA_TRUE(arg1)
    switch (op) {
        case OP_NEQ:
        case OP_GT:
             return(boolmvt_bop(arg2,OP_NOT));
        case OP_LT:
             return(boolmvt_bop(arg2,OP_FALSE));
        case OP_GE:
        case OP_MAX:
             return(boolmvt_bop(arg2,OP_TRUE));
...
int  op;                       /* eine compare Operation                     */
{
  ...
  int    i, *ip, *ipnew;       /* "i" == integer                             */
  double r, *rp, *rpnew;       /* "r" == real                                */
...
int op;                    /* eine compare Operation                         */
{
  ...
  if (R_DESC((*arg2),type) != TY_DIGIT)  /* die Matrix / der Vektor / der    */
    return(0);                           /* TVektor ist nicht vom Typ DIGIT  */
...
int op;                      /* eine compare Operation                       */
{
  ...
  if (R_DESC((*arg2),type) != TY_STRING) /* wenn keine Matrix / Vektor /     */
    return(0);                           /* TVektor vom Typ STRING vorliegt  */
...
int op;                     /* eine compare Operation                        */
{
  ...
  switch(class2) {
    ...
    case  C_VECTOR : if ((ncols = R_MVT(DESC(arg1),ncols,class1)) !=
                                  R_MVT(DESC(arg2),ncols,class2))
                       return(0);                /* nicht dieselbe Dimension */
                     nrows = 1;
                     break;
