 ...
 while ( i <= j)
  {
   ...
   hilf->var = (PTR_DESCRIPTOR)name[i];
   hilf->nr = LBV[i];
   hilf->next = VAR;
   VAR = (list *)hilf;
   i++;
   ...
   /* Markierung auf S_m legen */
    ...
    if ( index == 1)
    {
     ...
     /* Startausdruck darf nicht traversiert werden                   */
       ...
       POPSTACK(S_e);                         /* LIST(n) von S_e entfernen */
...
if ( T_PROTECT(x) ||                          /* Variable */       
   ...
   /* arity=0 => DOLLAR auf S_m */
    ...
    POPSTACK(S_m);                                      /* DOLLAR entfernen */
    PTRAV_A_E;                 /* Ausgangssituation auf dem E - Stack wieder*/
                               /*                                herstellen */
   
    if ( index == 1 )
    { 
       POPSTACK(S_e);                          /* LREC(2) von S_e entfernen */
