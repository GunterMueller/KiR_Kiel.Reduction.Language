/* commandos                                 ccommand.c */
/* letzte aenderung:    22.06.88                        */

#define iscmd  1 /* fuer commandos */
...
static int getonoff(buf)
 char *buf;
 {
  ...
  if (isdigit(*buf)) {
    ...
    if (*buf == 'o') c = *(buf+1) == 'n';   /* vorsicht : kein on/off ! */
    else             c = FAILURE;
  }
  return(c);
 }
...
#else
   erg = FALSE;           /* bei LETREC kein switch !          */
#endif
   return(erg);
 }
   ...
   if ((n = cmdsearch(&buf,cmdtb,CMDTBLEN)) != FAILURE) {
     switch(c = cmdtb[n].cnmb) {
      case c_special:
        ...
        back  = isfctvers;           /* rueckuebersetzung    */
        if (*buf  && sscanf(buf,"%d",&d) == 1) {
          rca = d;                    /* 1. parameter : red.-counter  */
          buf = skipword(buf);
          if (*buf && sscanf(buf,"%d",&d) == 1) {
            back  = d;                /* 2. par.: rueckuebersetzung   */
            buf = skipword(buf);
            if (*buf && sscanf(buf,"%d",&d) == 1) {
              tr   = d;               /* 3. par.: trace tored         */
              buf = skipword(buf);
              if (*buf && sscanf(buf,"%d",&d) == 1) {
                dspl = d;             /* 4. par.: anzeigen stacks     */
                buf = skipword(buf);
                if (*buf && sscanf(buf,"%d",&d) == 1) {
...
*/
             ...
             if (*buf == '\0') {
               switch(c) {
                 ...
                 case c_save_graph :
                   ...
                   if (n) {
                     Changes = YES;	/* erzwingt neu-uebersetzen	*/
      ...
      case c_prepro  :
        ...
        if (name[0]) {
          chkfilname(name,FN_PRE);	/* immer creat ?	*/
          /* upper(editfil[FN_PRE]);	namen nicht mehr gross schreiben	*/
          if ((c = creat(editfil[FN_PRE],0777)) != FAILURE) {
            Parms.file_desc = c;
            d = edit6(topof(EST));
            erg = redc(rca,0,0,isfctvers,0,RED_PREP);
            (void)close(c);     /* datei wieder schliessen */
            if (erg == SUCCESS) {
          /*  ptr = editfil[FN_PRE];    dateiname, vollstaendig oder     */
              getbase(name,FN_PRE);
              ptr = name;            /* nur teilname ? */
              c = *ptr++;
              ...
              transport(SST,EST);    /* alter inhalt ? */
            }
            if (!batch) dispfa();
           }
          else {
...
*/
      ...
      case c_editlib :
        if (*buf  && sscanf(buf,"%d",&d) == 1  &&  0 <= d  &&  d < FN_ANZ) {
          ...
          if (*buf) {
            ...
            /* ev. editieren der editlib mit getline ?         */
      ...
      case c_initfile:
        if (*buf) {
          ...
          /* ev. editieren des initfile mit getline ?         */
          strcpy(buf,initfile);
...
*/      clrmsg(INROW,INCOL);
      ...
      case c_store:
        ...
        if (name[0]) {
          copysted(EST,RST);       /* achtung: hier werden die editinfo         */
          clearstack(R1ST);        /* geloescht !                               */
...
*/      clrmsg(INROW,INCOL);
      ...
      case c_read   :
        ...
        break;
      case c_switch : (void)swt_vers(); /* kein break wg. vers_msg ! */
