...
DBUG_ENTER ("freeheap");
 ...
 { T_HBLOCK *_h_ = (T_HBLOCK *) p; 
                      if (_h_ != NULL) { 
#if (nCUBE && D_MESS && D_MHEAP)
    MPRINT_HEAP_FREE(D_MESS_FILE, M_TIMER, PROCESS_NR, PROCESS_PID, p, (int)(_h_-1)->ref);
        ...
        if (free_flg)
         {
          if (!buffer_flg)                        
           ...
           {
#if nCUBE
             fprintf(stderr,"node %d: Heap compaction successful: %d bytes free (last block is %d bytes); %d descriptors out of %d used\n", proc_id,
#else
             fprintf(stderr,"Heap compaction successful: %d bytes free (last block is %d bytes); %d descriptors out of %d used\n", 
#endif
...
DBUG_ENTER ("hpchain");
        ...
        for(p=hpbase ; p->next > p ; p=p->next) {
               ASSERT(((p>=hpbase) && (p<=hpend)),"hpchain vorn Arsch!");
        }
...
DBUG_ENTER ("hpchain2");
        ...
        for(p=hpbase ; p->next > p ; p=p->next) {
               ASSERT(((p>=hpbase) && (p<=hpend)),"vor message:hpchain vorn Arsch!");
        }
...
DBUG_ENTER ("hpchain_inter");
        ...
        for(p=hpbase ; p->next > p ; p=p->next) {
	  if (!((p>=hpbase) && (p<=hpend)))
	    fprintf(stderr,"Heap chain vorn Arsch!\n");
        }
        if (!(testbusy(hpend)))
	  fprintf(stderr,"Heap chain vorn Arsch!\n");
