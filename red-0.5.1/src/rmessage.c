...
#if D_MESS
#ifdef M_DEMO
 DBUG_PRINT("HOST", ("Ich installiere mdencred !"));
 if (rexecl(subcube,"mdencred","mdencred",0).np_pid == -1)
   post_mortem ("init_ncube: cannot install mdencred !");
#else
 DBUG_PRINT("HOST", ("Ich installiere mncred !"));
 if (rexecl(subcube,"mncred","mncred",0).np_pid == -1)
   post_mortem ("init_ncube: cannot install mncred");
#endif
#else
 DBUG_PRINT("HOST", ("Ich installiere ncred !"));
 if (rexecl(subcube,"ncred","ncred",0).np_pid == -1)
   post_mortem ("init_ncube: cannot install ncred");
#endif
#else
#if D_MESS
 DBUG_PRINT("HOST", ("Ich installiere mdncred !"));
 if (rexecl(subcube,"mdncred","mdncred",0).np_pid == -1)
   post_mortem ("init_ncube: cannot install mdncred !");
#else
 DBUG_PRINT("HOST", ("Ich installiere dncred !"));
 if (rexecl(subcube,"dncred","dncred",0).np_pid == -1)
   post_mortem ("init_ncube: cannot install dncred !");
#endif
#endif
}
...
int dim;
{int counter = 0,src;
 MESSAGE_TYPE type = MT_NCUBE_READY;

 while(counter < (1 << dim))
   {src = -1;
    nread(cube,NULL,0,(long *)&src,(int *) &type);
    counter++;}}
...
{int src = -1, type = -1;
 ...
 while (ntest(cube,&src,&type) >= 0)
   {nread(cube,NULL,0,&src,&type);
    src = type = -1;}}
...
/***** wait for merging finish *****/
  ...
  nread(cube,(char *)&buffer, 0, &src, &type);
...

{
 ...
 nread(cube,(char *)&buffer, 100, &src, &type);
...
/***** initialize nodes and start merging *****/
 ...
 for (i = 1; i < (1 << cube_dim); i++) {
   ...
   sprintf (line, "rcp /tmp/measure_node_uid%ld_%i %s:/tmp", getuid(), i-1, m_remote_host);
   ...
   sprintf (line, "rcp /tmp/measure_node_uid%ld_%i %s:/tmp", getuid(), (1 << cube_dim)-1, m_remote_host);
   DBUG_PRINT ("S_MERGE", ("copy command : %s", line));
   if ((m_super_value = system(line)) != 0) {
     printf (" command failed: %s, return: %d\n", line, m_super_value);
     host_flush();
  ...
  sprintf (line, "rcp %s %s:/tmp", mess_file, m_remote_host);
  ...
  if (m_del_files) {
    sprintf (line, "rm %s /tmp/measure_node_uid%ld_*", mess_file, getuid());
...
#ifdef M_BINARY
  sprintf (line, "rsh %s /home/majestix/rs/red/src/m_remote_merge %s %s %d 1 %d", m_remote_host, &mess_file[5], m_target_dir, (1 << cube_dim), m_del_files);
#else
  sprintf (line, "rsh %s /home/majestix/rs/red/src/m_remote_merge %s %s %d 0 %d", m_remote_host, &mess_file[5], m_target_dir, (1 << cube_dim), m_del_files);
#endif
