 ...
 case MT_RESULT:
  ...
  m_type = "MT_PARAM";
    ...
    case MT_CLOSE_MEASURE:
      ...
      if (proc_id == 0) {       /* i am no 0 ! */
        ...
        for (i = 0; i < (1 << cube_dim); i++) {
          sprintf (name_line, "%s_uid%ld_%i", M_MEASURE_NODE_FILE, getuid(), i);         
    ...
    case MT_OPEN_MEASURE:
      ...
      sprintf (d_mess_name, "%s_uid%ld_%i", M_MEASURE_NODE_FILE, getuid(), m_super_nr);
      sprintf (m_super_name, "/tmp/measure_node_uid%ld_%i", getuid(), m_super_nr);
