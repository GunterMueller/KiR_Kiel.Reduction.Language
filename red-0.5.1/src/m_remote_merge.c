/* remote merging program
   ...
   arguments:    filename for appending   (in /tmp)
                 target directory
                 number of files in /tmp
                 0 or 1 for ascii/binary
                 0 or 1 for deleting /tmp files
*/
...

{
  ...
  for (i = 0; i < m_number; i++) {
    sprintf (m_name, "/tmp/measure_node_uid%ld_%i", getuid(), i);
...

{
  ...
  for (i = 0; i < m_number; i++) {
    fread(&m_werte[i], sizeof(double), 1, m_files[i]);
  ...
  while (m_noch_da) {
    ...
    fwrite(&m_werte[take_me], sizeof(double), 1, m_result);
    ...
    fread(&m_werte[take_me], sizeof(double), 1, m_files[take_me]);
...

{
 ...
 strcpy(m_target_dir, argv[2]);
 sscanf (argv[3], "%d", &m_number);
 sscanf (argv[4], "%d", &m_binary);
 sscanf (argv[5], "%d", &m_del_files);
 ...
 sprintf (m_system, "mv /tmp/%s %s", m_file_name, m_target_dir);
 ...
 if (m_del_files) {
   sprintf (m_system, "rm /tmp/measure_node_uid%ld_*", getuid());
   DBUG_PRINT ("RMERGE", ("command: %s", m_system));
