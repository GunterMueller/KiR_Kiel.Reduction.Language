...
#define PP_PAPER(c)	((c) & PPPAPER)		/*stt*/
...
extern NO_EXTERNAL_CMD; /* External (remote call) */
#define IF_NOEXTERNALCMD if (NO_EXTERNAL_CMD)
#define IF_EXTERNALCMD(code) if (NO_EXTERNAL_CMD==0) code

#ifdef X_UDI
#define SET_EXTERNAL_ERROR(errmsg) strcpy(EXT_ERRORMSG,errmsg);EXT_ERROR=1
#else
#define SET_EXTERNAL_ERROR(errmsg) message(MROW,MCOL,errmsg)
#endif
