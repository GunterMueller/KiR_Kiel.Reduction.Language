/* $Log: rbibfunc.c,v $
 * Revision 1.3  1995/05/22  09:02:07  rs
...
/* copied from lneu-version by RS 12.5.1993 */
...
/* calls     : digit_ ***                                                    */
/*****************************************************************************/
void pirec(s,ax1,bx1,cx1,ax2,bx2,cx2,prz1,prz2,prodz1,prodz2,prodn,result,quot)
...
#if DEBUG
if (debconf) printf("\n\n\n trigdesc: *** end sts_desc initalization of permanent descriptor
s  ***\n");
#endif
...
#if DEBUG
  if (debconf) printf("\n\n\n expdesc: *** end sts_dec initalization of permanent descriptor
s  ***\n");
#endif
                        ...
                        quot;               /* evaluated expression per step  */
{
  ...
  if (LT(hlp1,_digRG)) {                    /*   relative error  <  _digRG    */
     ...
     DBUG_RETURN(sinrec(hlp1, hlp2, hlp3, hlp4, hlp5, hlp6));
  }
...
register PTR_DESCRIPTOR arg;
{
 ...
 /* ---> car INC_REFCNT extracted from functioncall */
     ...
     DBUG_RETURN(sinrec(_dig8,_dig110,hlp3,_dig39916800,hlp5,hlp6));
  }
...
PTR_DESCRIPTOR n;
{
  ...
  if (LT(hlp1,_digRG)) {
    ...
    DBUG_RETURN(lnrec(hlp1, hlp2, hlp3, hlp4, hlp5));
  }
...
register PTR_DESCRIPTOR arg;
{
  ...
  INC_REFCNT(_dig3);
  INC_REFCNT(_dig1);
  DBUG_RETURN(lnrec(hlp1,_dig5,_dig3,_dig1,hlp2));
...
register PTR_DESCRIPTOR arg,n;
{
  ...
  if (!LT(arg,_dig0p707)) {
    ...
    DBUG_RETURN(hlp3);                                  /* return eval. ln(x)     */
  }
  else {
    ...
    DBUG_RETURN(lntrans1(hlp1, hlp2));                  /* recursive function call*/
  }
...
register PTR_DESCRIPTOR arg,n;
{
  ...
  if (LE(arg,_dig1p414)) {
    ...
    DBUG_RETURN(lntrans2(hlp1,hlp2));                   /* recursive function call*/
  }
...
PTR_DESCRIPTOR n;
{
  ...
  if (LT(hlp1,_digRG)) {
    ...
    DBUG_RETURN(hlp2);                                /* return result of exp(x)  */
  }
  else {
    DEC_REFCNT(hlp1);                            /* ----- dealloc hlp1 ----- */
    ...
    DBUG_RETURN(exprec(hlp1, hlp2, hlp3, hlp4, hlp5));/* recursive function call  */
  }
...
register PTR_DESCRIPTOR arg;
{
 ...
 /* ---> car INC_REFCNT extracted from functioncall */
  ...
  INC_REFCNT(_dig1);
  INC_REFCNT(arg);
  DBUG_RETURN(exprec(powx,_dig2,_dig3,_dig1,arg));

} /*** end of function expfkt ***/

/*****************************************************************************/
/*                  power2     (2 power x  function)                           */
/* ------------------------------------------------------------------------- */
/* function  :  is the essential recursive function for evaluation of        */
/*              2 power n                                                    */
...
/* called by :  exptrans                                                     */
/*****************************************************************************/
PTR_DESCRIPTOR power2(result,n)
register PTR_DESCRIPTOR  result,n;
{
  ...
  DBUG_RETURN(result);                              /* return result of 2 power n */

} /*** end of function power2 ***/
...
/* returns   :  pointer to descriptor of argument                            */
/* calls     :  lnfkt, power2, expfkt                                          */
/* called by :  red_exp                                                      */
/*****************************************************************************/
PTR_DESCRIPTOR exptrans(arg)
register PTR_DESCRIPTOR arg;
{
  PTR_DESCRIPTOR /* y, RS June 1993 */ m;
 ...
 /* ---> car INC_REFCNT extracted from functioncall */
    INC_REFCNT(_dig1);
    TEST(hlp2 = power2(_dig1,m)); /* power2(_dig1,m)         */
  }

  TEST(hlp3 = MUL(hlp2,hlp1));                     /* power2(m)  * exp(g)      */
  DEC_REFCNT(hlp1);                                /* --- dealloc hlp1 ----- */
  DEC_REFCNT(hlp2);                                /* --- dealloc hlp2 ----- */
